...
from pysaucenao.errors import (
    ...
    TooManyFailedRequestsException,
...
class SauceNao:
    ...
    def __init__(
        ...
        *,
        ...
        db_mask: typing.Optional[int] = None,
        db_mask_disable: typing.Optional[int] = None,
        db: int = 999,
        results_limit: int = 6,
        min_similarity: float = 50.0,
        test_mode: int = 0,
        strict_mode: bool = True,
        priority: typing.Optional[typing.List] = None,
    ...
    async def from_url(self, url: str) -> SauceNaoResults:
        ...
        async with aiohttp.ClientSession(
            loop=self._loop, connector=self.connector
        ) as session:
    ...
    async def from_file(
        self, path_or_fh: typing.Union[str, typing.IO]
    ) -> SauceNaoResults:
        """
        Look up the source of an image on the local filesystem
        Args:
            path_or_fh (typing.Union[str, typing.IO): Path to the file to open or a file like object
        ...
        async def _post(_fh: typing.IO):
            ...
            async with aiohttp.ClientSession(
                ...
                self._log.debug(
                    f"Executing SauceNAO API request on local file: {path_or_fh}"
                )
                _status_code, _response = await self._post(
                    _session, self.API_URL, params
                )
    ...
    def _verify_request(self, status_code: int, data: dict) -> None:
        ...
        if status_code == 200:
            ...
            if self._strict_mode and (
                self.params.get("api_key") and not header["account_type"]
            ):
            ...
            if header["status"] != 0:
                ...
                if header["status"] < 0:
                    self._log.warning(
                        f"Non-zero status code returned: {header.get('status')}"
                    )
