...
class SauceNaoResults:
    ...
    def __init__(
        self,
        response: dict,
        min_similarity: typing.Optional[float] = None,
        ...
        self.results: typing.List[GenericSource] = [
            self._process_result(r) for r in self._results
        ]
    ...
    def _sort_results(self) -> None:
        ...
        if self._min_similarity:
            self._results = [
                ...
                if float(r["header"]["similarity"]) > self._min_similarity
            ]
        ...
        tolerance = (
            max([float(r["header"]["similarity"]) for r in self._results])
            - self._priority_tolerance
...
class TestResults:
    ...
    def __init__(
        self, response: dict, error: typing.Optional[SauceNaoException] = None
    ):
        self.error = error
...
class GenericSource:
    ...
    def __init__(self, header: dict, data: dict):
        ...
        self.index: typing.Optional[
            str
        ] = None  # The name of the index pulled from. See INDEXES above
    ...
    def _parse_data(self, data: dict):
        ...
        if "title" in data:
            ...
            self.title = data[
                "source"
            ]  # Sometimes this is a URL, sometimes it's a title. ¯\(°_o)/¯
...
class AnimeSource(VideoSource):
        ...
        async with aiohttp.ClientSession(
            ...
            try:
                response = await session.get(
                    f"https://relations.yuna.moe/api/ids?source=anidb&id={self.data.get('anidb_aid')}"
                )
                ...
                self._log.error(
                    f"yuna.moe server is returning a {error.status} error code"
                )
            except aiohttp.ClientError:
                self._log.error(
                    "yuna.moe server appears to be down or is not responding to our requests"
                )
    ...
    def _id_check(self, index: typing.Optional[str] = None):
        if self._ids is None:
            raise IndexError(
                "You must run the load_ids() method before accessing this property"
            )
